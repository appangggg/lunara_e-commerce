...
<div class="px-margin-mobile md:px-margin-desktop py-12 max-w-2xl mx-auto min-h-screen">
    ...
    <div class="mb-8 border-b border-outline-variant/30 pb-4 flex items-center justify-between">
        <div>
            <h1 class="font-display-lg text-headline-md font-bold uppercase tracking-tight text-on-background">Deploy New Item</h1>
    ...
    <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-surface-container-low border border-outline-variant/30 p-6 rounded-lg space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" required value="{{ old('name') }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background focus:ring-primary focus:border-primary transition-colors">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="price" name="price" required min="0" value="{{ old('price') }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background focus:ring-primary focus:border-primary transition-colors">
                    @error('price')<span class="text-error text-sm mt-1 block">{{ $message }}</span>@enderror
                </div>
                <div>
                    <label for="stock" class="block font-label-bold text-label-sm uppercase tracking-widest text-on-background mb-2">Initial Stock</label>
                    <input type="number" id="stock" name="stock" required min="0" value="{{ old('stock') }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background focus:ring-primary focus:border-primary transition-colors">
            ...
            <div>
                ...
                <p class="text-[10px] text-on-surface-variant mt-1 uppercase tracking-widest">Max upload size: 2MB. Supported formats: JPG, PNG, WEBP.</p>
