...
<div class="max-w-4xl mx-auto py-12">
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex space-x-4">
            ...
            <form action="{{ route('admin.drops.destroy', $drop->id) }}" method="POST" onsubmit="return confirm('WARNING: Are you sure you want to permanently erase this scheduled drop?');">
                @csrf
                @method('DELETE')
    ...
    <form method="POST" action="{{ route('admin.drops.update', $drop->id) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="bg-surface-container-low border border-outline-variant/30 p-6 rounded-lg space-y-6">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name', $drop->name) }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background">
                </div>
                <div>
                    <label class="block font-label-bold text-label-sm uppercase tracking-widest text-on-background mb-2">Top Label</label>
                    <input type="text" name="label" value="{{ old('label', $drop->label) }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background">
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="date_label" required value="{{ old('date_label', $drop->date_label) }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background">
                </div>
                <div>
                    ...
                    <select name="status" required class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background">
                        ...
                        <option value="live" {{ $drop->status == 'live' ? 'selected' : '' }}>Live</option>
                        <option value="archived" {{ $drop->status == 'archived' ? 'selected' : '' }}>Archived</option>
                    </select>
                </div>
                <div>
                    <label class="block font-label-bold text-label-sm uppercase tracking-widest text-on-background mb-2">Price Label</label>
                    <input type="text" name="price_label" value="{{ old('price_label', $drop->price_label) }}" class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background">
                </div>
            </div>

            <div>
                <label class="block font-label-bold text-label-sm uppercase tracking-widest text-on-background mb-2">Description</label>
                <textarea name="description" rows="3" required class="w-full bg-surface border border-outline-variant rounded p-3 text-on-background">{{ old('description', $drop->description) }}</textarea>
            </div>
            <div>
                ...
                @if($drop->image_url)
                    <img src="{{ $drop->image_url }}" alt="Current Image" class="w-24 h-24 object-cover rounded mb-4">
                @endif
